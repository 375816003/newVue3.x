...
(function (webim) {
  ...
  var proto_sendMsg = function (msg, cbOk, cbErr) {
    ...
    for (var i in msg.elems) {
      ...
      if (!msgType || msgType === undefined) { //马静 todo
        continue;
      }
      switch (msgType) {
  ...
  Msg.Elem.Images.prototype.toHtml = function () {
    ...
    return "<img src='" + smallImage.getUrl() + "#" + bigImage.getUrl() + "#" + oriImage.getUrl() + "' style='CURSOR: hand' id='" + this.getImageId() + "' bigImgUrl='" + bigImage.getUrl() + "' @click='imageClick' />";
  ...
  var MsgManager = new function () {
    ...
    var handlerGroupSystemMsgs = function (groupSystemMsgs, isNeedValidRepeatMsg) {
      for (var k in groupSystemMsgs) {
        ...
        console.log(groupReportTypeMsg, '领略到了的劳动力');
        if (groupReportTypeMsg === undefined) {
          continue;
        }
    ...
    var handlerOrdinaryAndTipC2cMsgs = function (eventType, C2cMsgArray) {
      ...
      for (var i in msgInfos) {
        ...
        for (var mi in msgInfo.MsgBody) {
          ...
          if (!msgBody.MsgType || msgBody.MsgType === undefined) { //马静 todo
            // return;
            continue;
          }
          switch (msgType) {
